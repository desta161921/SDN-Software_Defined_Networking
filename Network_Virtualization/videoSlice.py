...
class VideoSlice (EventMixin):
    ...
    def __init__(self):
        ...
        self.portmap = {('00-00-00-00-00-01', EthAddr('00:00:00:00:00:01'),
    ...
    def _handle_PacketIn (self, event):
        ...
        def forward (message = None):
            ...
            if packet.dst.is_multicast:
                ...
                try:
                    sliceLogic = (this_dpid, packet.src, packet.dst, packet.find('tcp').dstport)
                    if not self.portmap.get(sliceLogic):
                        sliceLogic = (this_dpid, packet.src, packet.dst, packet.find('tcp').srcport)
                        if not self.portmap.get(sliceLogic):
                            raise AttributeError

                    ndpid = self.portmap[sliceLogic]
                    log.debug("install: %s output %d" % (str(sliceLogic), self.adjacency[this_dpid][ndpid]))
